...
class TypeMappings {
    ...
    static DataType resolveType(AnnotatedField<?> t) {
        if (t.getJavaMember().getType().isAssignableFrom(List.class)) {
            String typeName = ((ParameterizedType) t.getJavaMember().getGenericType()).getActualTypeArguments()[0].getTypeName();
            return DataType.list(typeMappings.get(typeName));

        } else if (t.getJavaMember().getType().isAssignableFrom(Map.class)) {
            String keyTypeName = ((ParameterizedType) t.getJavaMember().getGenericType()).getActualTypeArguments()[0].getTypeName();
            String valueTypeName = ((ParameterizedType) t.getJavaMember().getGenericType()).getActualTypeArguments()[1].getTypeName();
            ...
            return Optional.ofNullable(typeMappings.get(t.getJavaMember().getType().getCanonicalName())).orElseThrow(
                       () -> new IllegalStateException("Unsupported type " + t.getJavaMember().getType().getCanonicalName()));
        }
    }
}
